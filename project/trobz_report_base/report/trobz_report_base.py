...
from .amount_to_text_vn import amount_to_text_vi as amount2textvn
...
class Parser(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.partner_env = env['res.partner']
        self.currency_env = env['res.currency']
    ...
    def convert_amount(self, date, amount,
        ...
        context = {'date': date}
        self.amount_converted = self.currency_env.compute(
            from_currency_id, to_currency_id,
            amount, context=context)
        to_currency = \
            self.currency_env.browse(to_currency_id)
        self.amount_converted = self.currency_env.round(
    ...
    def addr_get(self, partner_id, address_type='contact'):
        ...
        if not partner_id:
            return result
        partner_info = \
            self.partner_env.browse(partner_id)
        if not partner_info:
            return result
    ...
    def amount_to_text_vi(self, nbr, lang='vi', currency='VND'):
        return amount2textvn(nbr, lang, currency)
